...
for (const item of items) {
  if (!item.id || ids.has(item.id)) errors.push(`ID faltante o duplicado: ${item.id}`);
  ids.add(item.id);
  if (!allowedStatuses.has(item.status)) errors.push(`${item.id}: estado desconocido ${item.status}`);
  if (!releaseIds.has(item.release)) errors.push(`${item.id}: release desconocido ${item.release}`);
  if (!project.sizeWeights[item.size]) errors.push(`${item.id}: tamaño desconocido ${item.size}`);
  for (const dep of item.dependencies || []) {
    if (!items.some(i => i.id === dep)) errors.push(`${item.id}: dependencia inexistente ${dep}`);
  }
  if (item.status === 'in_progress' && (item.progress === null || item.progress === undefined)) {
    errors.push(`${item.id}: in_progress requiere progress`);
  }
}

if (errors.length) {
  console.error('Errores de datos de project management:\n- ' + errors.join('\n- '));
  process.exit(1);
}
...
const weightedProgress = list => {
  ...
  if (!total) return 0;
  return Math.round(list.reduce((s, i) => s + weight(i) * progressValue(i), 0) / total);
};
...
const statusIcon = {
  proposed: '○', ready: '◉', in_progress: '▶', blocked: '⛔', review: '◆', done: '✓', deferred: '–'
};
...
const counts = Object.fromEntries(project.statusVocabulary.map(s => [s, items.filter(i => i.status === s).length]));
const active = items.filter(i => ['in_progress','review','blocked'].includes(i.status));
const ready = items.filter(i => i.status === 'ready');
const p0Open = items.filter(i => i.priority === 'P0' && i.status !== 'done' && i.status !== 'deferred');
...
if (!active.length) statusMd += `No hay tareas marcadas como en progreso, revisión o bloqueadas.\n\n`;
else {
  statusMd += `| ID | Estado | Trabajo | Responsable | Progreso |\n|---|---|---|---|---:|\n`;
  for (const i of active) statusMd += `| ${i.id} | ${statusIcon[i.status]} ${statusLabel[i.status]} | ${i.title} | ${i.owner} | ${progressValue(i)}% |\n`;
  statusMd += `\n`;
}
statusMd += `### Próximas tareas listas\n\n`;
statusMd += `| ID | Prioridad | Trabajo | Release | Dependencias |\n|---|---|---|---|---|\n`;
for (const i of ready.slice(0,15)) statusMd += `| ${i.id} | ${i.priority} | ${i.title} | ${i.release} | ${(i.dependencies||[]).join(', ') || '—'} |\n`;
statusMd += `\n### P0 abiertos\n\n`;
statusMd += `| ID | Estado | Trabajo | Release |\n|---|---|---|---|\n`;
for (const i of p0Open) statusMd += `| ${i.id} | ${statusLabel[i.status]} | ${i.title} | ${i.release} |\n`;
...
for (const r of bySequence) {
  ...
  const state = list.length && list.every(i=>i.status==='done') ? 'Terminada' : (r.id===project.currentRelease ? 'Actual' : (r.sequence < (currentRelease?.sequence ?? 0) ? 'Parcial' : 'Planificada'));
...
for (const r of bySequence) {
  ...
  if (r.exitCriteria.length) for (const c of r.exitCriteria) roadmapMd += `- [ ] ${c}\n`;
  else roadmapMd += `- Sin criterios comprometidos.\n`;
  roadmapMd += `\n### Work items\n\n| ID | Tipo | Prioridad | Estado | Tamaño | Trabajo |\n|---|---|---|---|---|---|\n`;
  for (const i of list) roadmapMd += `| ${i.id} | ${i.type} | ${i.priority} | ${statusIcon[i.status]} ${statusLabel[i.status]} | ${i.size} | ${i.title} |\n`;
  roadmapMd += `\n`;
}
...
for (const r of bySequence) {
  backlogMd += `## ${r.name}\n\n`;
  const list = byRelease(r.id).sort((a,b)=>a.priority.localeCompare(b.priority) || a.id.localeCompare(b.id));
  backlogMd += `| ID | Tipo | Epic | Prioridad | Estado | Tamaño | Responsable | Dependencias | Trabajo |\n|---|---|---|---|---|---|---|---|---|\n`;
  for (const i of list) backlogMd += `| ${i.id} | ${i.type} | ${i.epic} | ${i.priority} | ${statusIcon[i.status]} ${statusLabel[i.status]} | ${i.size} | ${i.owner} | ${(i.dependencies||[]).join(', ') || '—'} | ${i.title} |\n`;
  backlogMd += `\n`;
}
...
console.log(`Dashboard actualizado: ${items.length} work items, avance ${overall}%.`);
