...
function normalizeItem(raw, existing = null) {
  const item = {
    id: safeText(raw.id ?? existing?.id, 40).trim().toUpperCase(),
    type: safeText(raw.type ?? existing?.type, 20).trim().toUpperCase(),
    epic: safeText(raw.epic ?? existing?.epic, 100).trim(),
    title: safeText(raw.title ?? existing?.title, 300).trim(),
    priority: safeText(raw.priority ?? existing?.priority, 5).trim().toUpperCase(),
    status: safeText(raw.status ?? existing?.status, 30).trim(),
    release: safeText(raw.release ?? existing?.release, 100).trim(),
    size: safeText(raw.size ?? existing?.size, 5).trim().toUpperCase(),
    owner: safeText(raw.owner ?? existing?.owner, 100).trim() || 'TBD',
    dependencies: normalizeStringArray(raw.dependencies ?? existing?.dependencies).map(v => v.toUpperCase()),
    progress: raw.progress === '' || raw.progress === null || raw.progress === undefined
      ? null
      : Math.max(0, Math.min(100, Number(raw.progress))),
    acceptanceCriteria: normalizeStringArray(raw.acceptanceCriteria ?? existing?.acceptanceCriteria),
...
function validateData({ project, releases, items }) {
  ...
  if (!releaseIds.has(project.currentRelease)) errors.push(`project.json: currentRelease desconocido (${project.currentRelease}).`);

  for (const release of releases) {
    if (!release.id) errors.push('Release sin ID.');
    if (!release.name) errors.push(`${release.id || 'release'}: falta name.`);
    if (!Array.isArray(release.exitCriteria)) warnings.push(`${release.id}: exitCriteria debería ser una lista.`);
  }
  ...
  for (const item of items) {
    ...
    itemIds.add(item.id);
    if (!/^[A-Z][A-Z0-9]*-[0-9]{3,}$/.test(item.id || '')) warnings.push(`${item.id || 'sin-id'}: formato de ID no convencional.`);
    if (!item.title) errors.push(`${item.id}: falta title.`);
    if (!statuses.has(item.status)) errors.push(`${item.id}: estado desconocido (${item.status}).`);
    if (!releaseIds.has(item.release)) errors.push(`${item.id}: release desconocido (${item.release}).`);
    if (!priorities.has(item.priority)) errors.push(`${item.id}: prioridad desconocida (${item.priority}).`);
    if (!sizes.has(item.size)) errors.push(`${item.id}: tamaño desconocido (${item.size}).`);
    if (item.status === 'in_progress' && !Number.isFinite(item.progress)) errors.push(`${item.id}: in_progress requiere progress.`);
    if (item.status === 'review' && !Number.isFinite(item.progress)) warnings.push(`${item.id}: review debería indicar progress.`);
    if (item.status === 'blocked' && !String(item.notes || '').trim()) warnings.push(`${item.id}: bloqueada sin explicación en notes.`);
    if (item.status === 'done' && !(item.acceptanceCriteria || []).length) warnings.push(`${item.id}: terminada sin criterios de aceptación registrados.`);
  }

  for (const item of items) {
    for (const dep of item.dependencies || []) {
      if (!itemIds.has(dep)) errors.push(`${item.id}: dependencia inexistente (${dep}).`);
      if (dep === item.id) errors.push(`${item.id}: no puede depender de sí misma.`);
    }
  }
  ...
  function detectCycle(id, pathStack) {
    if (visiting.has(id)) {
      ...
      errors.push(`Dependencia circular: ${[...pathStack.slice(start), id].join(' → ')}.`);
      return;
    }
    if (visited.has(id)) return;
    visiting.add(id);
    pathStack.push(id);
    for (const dep of adjacency.get(id) || []) if (adjacency.has(dep)) detectCycle(dep, pathStack);
    pathStack.pop();
    visiting.delete(id);
    visited.add(id);
  }
...
function itemProgress(project, item) {
...
function weightedProgress(project, list) {
  const total = list.reduce((sum, item) => sum + (project.sizeWeights[item.size] || 0), 0);
  if (!total) return 0;
  const achieved = list.reduce((sum, item) => {
    const weight = project.sizeWeights[item.size] || 0;
    return sum + weight * itemProgress(project, item);
  }, 0);
  return Math.round(achieved / total);
}

function gitInfo() {
  const run = args => execFileSync('git', args, { cwd: repoRoot, encoding: 'utf8', windowsHide: true }).trim();
  try {
    return {
      available: true,
    ...
    return { available: false, branch: 'no disponible', status: '', lastCommit: '', error: safeText(error.message, 500) };
  }
}
...
function parseRecentActivity(limit = 12) {
  ...
  const parts = text.split(/^##\s+/m).slice(1).map(section => {
...
function buildState() {
  ...
  const counts = Object.fromEntries((project.statusVocabulary || []).map(status => [status, items.filter(i => i.status === status).length]));
  ...
  const releaseProgress = releases.map(release => {
    ...
    return { ...release, progress: weightedProgress(project, releaseItems), itemCount: releaseItems.length };
  });
  return {
    ...
    summary: {
      ...
      total: items.length,
      counts,
      p0Open: items.filter(i => i.priority === 'P0' && !['done', 'deferred'].includes(i.status)).length,
...
function createBackup(reason = 'save') {
  ensureDirectories();
  const folder = path.join(backupsDir, `${timestampFolder()}-${safeText(reason, 30).replace(/[^a-zA-Z0-9_-]/g, '-')}`);
  fs.mkdirSync(folder, { recursive: true });
  for (const file of Object.values(jsonFiles)) if (fs.existsSync(file)) fs.copyFileSync(file, path.join(folder, path.basename(file)));
  if (fs.existsSync(progressLogPath)) fs.copyFileSync(progressLogPath, path.join(folder, path.basename(progressLogPath)));
  const directories = fs.readdirSync(backupsDir, { withFileTypes: true })
    .filter(entry => entry.isDirectory())
    .map(entry => ({ name: entry.name, time: fs.statSync(path.join(backupsDir, entry.name)).mtimeMs }))
    .sort((a, b) => b.time - a.time);
  for (const old of directories.slice(MAX_BACKUPS)) fs.rmSync(path.join(backupsDir, old.name), { recursive: true, force: true });
  return folder;
}
...
function regenerateMarkdown() {
  if (!fs.existsSync(generatorPath)) throw new Error('No se encontró tools/update-dashboard.mjs.');
  const result = spawnSync(process.execPath, [generatorPath], { cwd: repoRoot, encoding: 'utf8', windowsHide: true });
  if (result.status !== 0) throw new Error((result.stderr || result.stdout || 'Falló la regeneración.').trim());
...
async function handleApi(req, url, res) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/validate') {
    ...
    return sendJson(res, state.validation.valid ? 200 : 422, { validation: state.validation });
  }

  if (req.method === 'POST' && url.pathname === '/api/regenerate') {
    const state = buildState();
    if (!state.validation.valid) return sendJson(res, 422, { error: 'Los datos contienen errores.', validation: state.validation });
  ...
  if (req.method === 'POST' && url.pathname === '/api/work-items') {
    ...
    if (data.items.some(existing => existing.id === item.id)) return sendJson(res, 409, { error: `Ya existe ${item.id}.` });
    const proposed = { ...data, items: [...data.items, item] };
    const validation = validateData(proposed);
    if (!validation.valid) return sendJson(res, 422, { error: 'La tarea no supera la validación.', validation });
  ...
  if (req.method === 'PATCH' && itemMatch) {
    const body = await readBody(req);
    const data = readData();
    const index = data.items.findIndex(item => item.id === itemMatch[1].toUpperCase());
    ...
    const validation = validateData(proposed);
    if (!validation.valid) return sendJson(res, 422, { error: 'Los cambios no superan la validación.', validation });
  ...
  if (req.method === 'PATCH' && releaseMatch) {
    const body = await readBody(req);
    const data = readData();
    const index = data.releases.findIndex(release => release.id === releaseMatch[1]);
    if (index < 0) return sendJson(res, 404, { error: 'Release no encontrada.' });
    const updated = {
      ...data.releases[index],
      goal: body.goal === undefined ? data.releases[index].goal : safeText(body.goal, 2000).trim(),
      target: body.target === undefined ? data.releases[index].target : safeText(body.target, 100).trim(),
      exitCriteria: body.exitCriteria === undefined ? data.releases[index].exitCriteria : normalizeStringArray(body.exitCriteria)
    };
  ...
  if (req.method === 'POST' && url.pathname === '/api/progress') {
    ...
    if (!title) return sendJson(res, 422, { error: 'El título del registro es obligatorio.' });
    const sections = [
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    console.error(error);
    sendJson(res, error.statusCode || 500, { error: safeText(error.message || 'Error interno.', 2000) });
  }
});
...
server.on('error', error => {
  if (error.code === 'EADDRINUSE') console.error(`El puerto ${PORT} ya está en uso. Cierra la instancia anterior o define PM_DASHBOARD_PORT.`);
