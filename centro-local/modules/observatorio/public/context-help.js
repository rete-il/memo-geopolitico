...
function panelId(dialog) {
  return `${dialog.id}-context-help`;
}

function ensurePanel(dialog) {
  if (!dialog) return null;
  const existing = dialog.querySelector('.dialog-help-panel');
  if (existing) return existing;
  const body = dialog.querySelector('.dialog-body');
  if (!body) return null;
  const panel = document.createElement('aside');
  panel.id = panelId(dialog);
  panel.className = 'dialog-help-panel';
  panel.hidden = true;
  panel.setAttribute('role', 'region');
  panel.setAttribute('aria-live', 'polite');
  panel.innerHTML = `
    <header class="dialog-help-head">
      <div><p class="eyebrow">AYUDA CONTEXTUAL</p><h3 class="dialog-help-title">Orientación del campo</h3></div>
      <button type="button" class="icon close-dialog-help" aria-label="Cerrar ayuda">×</button>
    </header>
    <div class="dialog-help-body"></div>`;
  body.prepend(panel);
  return panel;
}

function buttonFor(key, label, dialog) {
  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'help-trigger';
  button.textContent = 'ⓘ';
  button.dataset.helpKey = key;
  button.dataset.helpSummary = HELP[key]?.meaning || '';
  button.setAttribute('aria-label', `Ayuda: ${label}`);
  button.setAttribute('aria-haspopup', 'true');
  button.setAttribute('aria-expanded', 'false');
  if (dialog) button.setAttribute('aria-controls', panelId(dialog));
  return button;
}
...
function attachField(selector, key) {
  ...
  label.append(buttonFor(key, title, dialog));
}
...
function attachSection(anchorSelector, key) {
  ...
  legend.append(buttonFor(key, title, dialog));
}
...
function renderPanel(panel, trigger, key) {
  const definition = HELP[key];
  if (!panel || !definition) return;
  const title = trigger.getAttribute('aria-label')?.replace(/^Ayuda:\s*/, '') || 'Orientación del campo';
  panel.querySelector('.dialog-help-title').textContent = title;
  panel.querySelector('.dialog-help-body').innerHTML = `${section('Qué significa', definition.meaning)}${section('Por qué importa', definition.contribution)}${questions(definition.questions)}${section('Ejemplo', definition.example)}${section('Evitar', definition.avoid)}`;
}
...
function openContextHelp(trigger) {
  const key = trigger?.dataset.helpKey;
  const dialog = trigger?.closest('dialog');
  const panel = ensurePanel(dialog);
  if (!trigger || !key || !panel) return;
  if (currentTrigger === trigger && !panel.hidden) {
...
export function initContextHelp() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (trigger) {
      ...
      openContextHelp(trigger);
      return;
    }
    const close = event.target.closest('.close-dialog-help');
    if (close) {
      event.preventDefault();
      event.stopPropagation();
      closeContextHelp(true);
    ...
    if (currentPanel && !currentPanel.hidden && !currentPanel.contains(event.target)) {
      closeContextHelp(false);
    }
  }, true);

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && currentPanel && !currentPanel.hidden) {
      event.preventDefault();
      event.stopPropagation();
  ...
  document.querySelectorAll('dialog').forEach((dialog) => {
    dialog.addEventListener('close', () => closeContextHelp(false));
  });

  console.info('[Observatorio] Ayuda contextual reconstruida · v0.4.0');
}
