(() => {
  ...
  if (!source || !Array.isArray(source.records)) {
    document.body.innerHTML = "<p style='padding:2rem;font-family:sans-serif'>No se pudo cargar el conjunto de datos.</p>";
    return;
  }

  const allRows = source.records.map((row) => ({ ...row }));
  const labels = Object.fromEntries((source.columns || []).map((c) => [c.key, c.label]));
  const excelFile = `data/${source.metadata.archivo_fuente}`;
  ...
  const uniqueSorted = (key) =>
    [...new Set(allRows.map((row) => String(row[key] || "").trim()).filter(Boolean))]
  ...
  const searchableText = (row) =>
    normalize([
      row.nombre, row.region, row.familia, row.funcion, row.perspectiva,
      row.orientacion, row.uso, row.corrobacion, row.estado, row.sede
    ].join(" "));
  ...
  const getFiltered = () => {
    ...
    const filtered = allRows.filter((row) => {
      ...
      if (state.perspectiva && row.perspectiva !== state.perspectiva) return false;
      if (state.confianza && row.confianza !== state.confianza) return false;
      if (state.estado && row.estado !== state.estado) return false;
      if (Number(row.puntuacion || 0) < Number(state.minScore || 0)) return false;
      return true;
    });
  ...
  const countBy = (rows, key) => {
    const map = new Map();
    rows.forEach((row) => {
      const value = String(row[key] || "Sin clasificar").trim() || "Sin clasificar";
  ...
  const renderKpis = (rows) => {
    ...
    const active = rows.filter((r) => ["Activo", "Nuevo recomendado"].includes(r.estado)).length;
  ...
  const donutColors = ["#2f75b5", "#1f6d70", "#d6a84b", "#c65911", "#a9b8c9", "#8b5cf6"];
  ...
  const renderCharts = (rows) => {
    ...
    renderDonut("confidence-donut", "confidence-legend", countBy(rows, "confianza"), rows.length);
    renderDonut("status-donut", "status-legend", countBy(rows, "estado"), rows.length);
  };
  ...
  const renderTable = (rows) => {
    ...
    $$(".details-btn", els.tbody).forEach((button) => {
    ...
    els.modalBody.innerHTML = `
      <section class="detail-section">
        ...
        <div class="detail-grid">
          ${detailItem(labels.url || "URL", row.url, true)}
          ${detailItem(labels.sede || "Sede", row.sede)}
          ${detailItem(labels.region || "Región", row.region)}
          ${detailItem(labels.idioma || "Idioma", row.idioma)}
          ${detailItem(labels.familia || "Familia", row.familia)}
          ${detailItem(labels.funcion || "Función", row.funcion)}
        </div>
      </section>
      <section class="detail-section">
        <h3>Marco institucional y geopolítico</h3>
        <div class="detail-grid">
          ${detailItem(labels.propiedad || "Propiedad", row.propiedad)}
          ${detailItem(labels.control || "Control", row.control)}
          ${detailItem(labels.orientacion || "Orientación", row.orientacion)}
          ${detailItem(labels.perspectiva || "Perspectiva", row.perspectiva)}
          ${detailItem(labels.confianza || "Confianza", row.confianza)}
          ${detailItem(labels.estado || "Estado", row.estado)}
        </div>
      </section>
      <section class="detail-section">
        <h3>Evaluación</h3>
        <div class="metric-grid">
          ${metric("Fiabilidad", row.fiabilidad)}
          ${metric("Independencia", row.independencia)}
          ${metric("Transparencia", row.transparencia)}
          ${metric("Rigor", row.rigor)}
          ${metric("Correcciones", row.correcciones)}
          ${metric("Noticia/opinión", row.separacion)}
          ${metric("Global", row.puntuacion)}
        </div>
      </section>
      <section class="detail-section">
        <h3>Uso analítico</h3>
        <div class="detail-grid">
          ${detailItem(labels.uso || "Uso recomendado", row.uso)}
          ${detailItem(labels.corroboracion || "Corroboración", row.corroboracion)}
          ${detailItem(labels.corroborar_con || "Corroborar con", row.corroborar_con)}
          ${detailItem(labels.observaciones || "Observaciones", row.observaciones)}
          ${detailItem(labels.referencia || "Referencia institucional", row.referencia, true)}
          ${detailItem(labels.fecha_revision || "Fecha de revisión", row.fecha_revision)}
        </div>
      </section>
    `;
  ...
  const activeFilterText = (rows) => {
    ...
    ["region", "familia", "funcion", "perspectiva", "confianza", "estado"].forEach((key) => {
      if (state[key]) active.push(`${key}: ${state[key]}`);
    });
    if (state.minScore > 0) active.push(`puntuación ≥ ${Number(state.minScore).toFixed(1)}`);
  ...
  const updateSortIndicators = () => {
    $$("[data-sort]").forEach((button) => {
      ...
      button.textContent = button.dataset.sort === state.sortKey
  ...
  const resetFilters = () => {
    Object.assign(state, {
      search: "", region: "", familia: "", funcion: "", perspectiva: "",
      confianza: "", estado: "", minScore: 0, page: 1
    });
    syncControls();
    render();
  };
  ...
  const initTheme = () => {
    ...
    els.theme.textContent = document.documentElement.dataset.theme === "dark" ? "☀️ Tema claro" : "🌙 Tema oscuro";
    els.theme.addEventListener("click", () => {
      const next = document.documentElement.dataset.theme === "dark" ? "light" : "dark";
      document.documentElement.dataset.theme = next;
      localStorage.setItem("media-dashboard-theme", next);
      els.theme.textContent = next === "dark" ? "☀️ Tema claro" : "🌙 Tema oscuro";
    });
  };
  ...
  const init = () => {
    ...
    setOptions(els.perspectiva, uniqueSorted("perspectiva"), "Todas las perspectivas");
    ...
    if (window.EMBEDDED_EXCEL_BASE64) {
      const binary = atob(window.EMBEDDED_EXCEL_BASE64);
      const bytes = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i += 1) bytes[i] = binary.charCodeAt(i);
      const excelBlob = new Blob([bytes], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
      });
      els.excelDownload.href = URL.createObjectURL(excelBlob);
    } else {
      els.excelDownload.href = excelFile;
    }
    els.excelDownload.download = source.metadata.archivo_fuente;
    els.dataUpdated.textContent =
      `Fuente: ${source.metadata.archivo_fuente} · ${source.metadata.total_fuentes} medios · revisión ${source.metadata.ultima_revision || "sin fecha"}`;
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && els.modal.classList.contains("is-open")) closeModal();
    });
