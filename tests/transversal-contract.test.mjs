...
test('el corpus canónico declara propiedad única y relaciones tipadas auditables', () => {
  ...
  assert.equal(data.relaciones_macroeventos.length, 114);
  assert.equal(new Set(data.relaciones_macroeventos.map((relation) => relation.id)).size, 114);
  assert.equal(rectors.flatMap((event) => event.senales).length, 75);
  assert.equal(
...
test('la proyección pública conserva mecanismos y referencias sin campos editoriales internos', () => {
  ...
  assert.equal(result.valid, true, result.errors.join('\n'));
  assert.equal(relations.length, 228);
